...
def compressing_loop(compressed_dir, script_dir, chunked_list):
    ...
    for jpg in chunked_list:
        ...
        new_path = str(os.path.join(new_dir, jpg)).replace('.jpg', '_compressed.jpg')
        ...
        with Image.open(fp=old_path) as image:
            ...
            # resize
            newsize = int(image.height/2), int(image.width/2)  # has no effect...?!

            # save with new name, new size and lower quality
            image.resize(newsize, Image.ANTIALIAS)
            image.save(new_path, quality=50)
            image.close()

            # examples:
            # test set: 83 MB 
            # quality=85 reduces to: 36.4 MB 43%
            # quality=50 reduces to: 16.9 MB 23%
...
def copying_loop(compressed_dir, script_dir, copy_targets):
    for copy_target in copy_targets:
        ...
        new_path = str(os.path.join(new_dir, copy_target)).replace('.jpg', '_compressed.jpg')
...
def main():
    try:
        ...
        if not jpg_compress_amout:
            ...
            #go!
            p1 = Process(target=compressing_loop, args=(compressed_dir, script_dir, chunk_1))
            p2 = Process(target=compressing_loop, args=(compressed_dir, script_dir, chunk_2))
            p3 = Process(target= compressing_loop, args=(compressed_dir, script_dir, chunk_3))
            p4 = Process(target= compressing_loop, args=(compressed_dir, script_dir, chunk_4))
